...
class Autoloader
{
    ...
    {
        if (strstr($className, 'LwPortalList')) {
            $path = $this->config["plugin_path"]["lw"] . "lw_portal_list";
            $filename = str_replace('LwPortalList', $path, $className);
        }
...
$db->setStatement("SELECT * FROM t:lw_info_portals ORDER BY name ");
$result = $db->pselect();
